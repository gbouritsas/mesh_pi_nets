...
class SpiralPolyAE(nn.Module):
    def __init__(self, filters_enc, filters_dec, latent_size, sizes, spiral_sizes, 
        ...
        for i in range(len(spiral_sizes)-1):
            if filters_enc[1][i]:
                self.conv.append(SpiralPoly(input_size, spiral_sizes[i], filters_enc[1][i],
            ...
            self.conv.append(SpiralPoly(input_size, spiral_sizes[i], filters_enc[0][i + 1],
                                        activation=self.activation, device=device,
                                        injection = self.injection, residual = self.residual,
                                        num_points =  sizes[i], order = self.order,
                                        normalize = self.normalize, model = self.model).to(device))
...
#         else:
        ...
        for i in range(len(spiral_sizes)-1):
            if i != len(spiral_sizes)-2:
                self.dconv.append(SpiralPoly(input_size, spiral_sizes[-2 - i], filters_dec[0][i + 1],
                                             activation=self.activation, device=device,
                                             injection = self.injection, residual = self.residual,
                ...
                if filters_dec[1][i+1]:
                    self.dconv.append(SpiralPoly(input_size, spiral_sizes[-2 - i], filters_dec[1][i + 1],
                                                 activation=self.activation, device=device,
                                                 injection = self.injection, residual = self.residual,
                                                 num_points = sizes[-2-i], order = self.order,
                                                 normalize = self.normalize, model = self.model).to(device))
