				...
				do
					...
					if ! ( [ $ORDER = 2 ] && [ $NORMALIZE = "2nd" ] ); then
						if ! ( ( [ $MODEL = "simple" ] || [ $ORDER = 2 ] ) && [ $RESIDUAL = "True" ] ); then
							...
							echo $(python spiral_pi_nets.py --name sliced --dataset COMA --mode test --order ${ORDER} --model ${MODEL}  --normalize ${NORMALIZE} --residual ${RESIDUAL} --activation ${ACTIVATION}  --results_folder TPAMI/${ORDER_NAME}_order_${MODEL}_norm_${NORMALIZE}${RESIDUAL_NAME}${ACTIVATION_NAME} --device_idx 5 --batch_size 16 --mm_constant ${1})
							i=$((i+1))
						fi
					fi
				done
